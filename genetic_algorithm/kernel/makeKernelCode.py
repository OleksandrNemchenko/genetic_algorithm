...
if __name__ == "__main__":
    ...
    try:
        ...
        if not all ([args.source_cpp_pattern_file, args.source_cpp_pattern, args.source_c_kernel_file, args.source_c_kernel_prefix, args.source_c_kernel_postfix, args.destination_cpp_file]):
            ...
            if not silence : print(f"- Load kernel code file {args.source_c_kernel_file}")
            kernel_file = load_text_file(args.source_c_kernel_file)
            
            if not silence : print(f"- Extract kernel code inside tags {args.source_c_kernel_prefix} .. {args.source_c_kernel_postfix}")
            kernel_code = get_kernel_code(kernel_file, args.source_c_kernel_prefix, args.source_c_kernel_postfix)
            
            if not silence : print(f"- Load C++ patter file {args.source_cpp_pattern_file}")
            ...
            if not silence : print(f"- Save destination C++ file {args.destination_cpp_file}")
